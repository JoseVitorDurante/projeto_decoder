...
public class SpecificationTemplate {
    ...
    public static Specification<LessonModel> lessonModuleId(final UUID moduleId){
        return ((root, query, criteriaBuilder) -> {
            ...
            return criteriaBuilder.and(criteriaBuilder.equal(module.get("moduleId"), moduleId), criteriaBuilder.isMember(lesson, lessonModules));
        });
    }
    ...
    public static Specification<CourseModel> courseUserId(final UUID userId){
        return (root, query, criteriaBuilder) -> {
            query.distinct(true);
            Join<CourseModel, CourseUserModel> coursesProd = root.join("courseUsers");
            return criteriaBuilder.equal(coursesProd.get("userId"), userId);
        };
    }
}
